...
export function cantidadTomada(producto) {
  ...
  if (tomas.length > 0) {
    return tomas.reduce((acc, toma) => acc + (Number(toma.Cantidad) || 0), 0)
  }
...
export function usePedidoProductosLibre() {
   ...
   * modo aún no existe.
   */
  function buildDetallesPayload(esEntradaDirecta = false) {
    return lineas.value.flatMap((producto) => {
      ...
      if (esEntradaDirecta) {
        return [{ ...base, Cantidad: Number(producto.Cantidad) || 0, IdLote: producto.IdLote }]
      }
