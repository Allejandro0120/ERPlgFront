        ...
        <v-row class="mt-2">
          ...
          <v-col cols="12" md="8">
            <v-row>
              ...
              <!-- Pedido normal: toma por lote/ubicación + faltante -->
              <template v-else-if="form.IdProducto">
          ...
          <!-- Columna del resumen -->
          <v-col v-if="form.IdProducto" cols="12" md="4">
...
<script setup>
  ...
  const disableConfirm = computed(() => {
    ...
    if (props.esEntradaDirecta) {
      if (Number(form.value.Cantidad) <= 0 || !form.value.IdLote) return true
  ...
  const hasChanges = computed(() => {
    if (isCreating.value) {
      if (props.esEntradaDirecta) return !!form.value.IdProducto || Number(form.value.Cantidad) > 0
      return !!form.value.IdProducto || tomas.value.length > 0
    }
  ...
  async function precargarLinea(linea) {
    form.value = {
      ...formInitial,
      ...linea,
    ...
    if (props.esEntradaDirecta && linea.IdProducto) {
      await cargarLotes(linea.IdProducto)
    }
  ...
  async function submitForm() {
    ...
    if (props.esEntradaDirecta) {
      if (Number(form.value.Cantidad) <= 0) {
        $toast.warning('Indica una cantidad mayor a 0 para agregar el producto.')
        return
      }
      if (!form.value.IdLote) {
        $toast.warning('Selecciona un lote para agregar el producto.')
        return
      }
    } else {
    ...
    const datosComunes = {
      ...
      ...(props.esEntradaDirecta
        ...
        : {
            ...
            CantidadFaltante: Number(form.value.CantidadFaltante) || 0,
          }),
    }
