<template>
  <div class="mt-2">
    ...
    <base-table-local
      ...
      :items-per-page="5"
    >
      <template #[`item.NombreSucursal`]="{ item }">
        <div class="py-2">
      ...
      <template #[`item.Habilitada`]="{ item }">
        <v-chip :color="item.Habilitada ? 'success' : 'error'" size="x-small" variant="tonal">
          <v-icon start size="12">
            {{ item.Habilitada ? "mdi-check-circle-outline" : "mdi-close-circle-outline" }}
          </v-icon>
          {{ item.Habilitada ? "Activa" : "Inactiva" }}
        </v-chip>
      </template>

      <template #[`item.Contacto`]="{ item }">
        <div class="py-2">
          <div class="text-body-2 text-grey-darken-2">{{ item.Telefono || "—" }}</div>
          <div class="text-caption text-grey">{{ item.CorreoGeneral || "Sin correo" }}</div>
        </div>
      </template>
    </base-table-local>
  </div>
...
const { sucursales, headers, rowActions, isReadonly, departamentos } = toRefs(props);

const deptoMap = computed(() =>
  Object.fromEntries(
    (departamentos.value || []).map((d) => [d.IdDepartamento, d.NombreDepartamento]),
  ),
);
