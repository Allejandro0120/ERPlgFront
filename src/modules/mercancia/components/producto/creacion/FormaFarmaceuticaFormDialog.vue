...
<script setup>
  ...
  async function submitForm() {
    ...
    const confirmed = await $confirm.warning({
      ...
      message: `¿Deseas crear la forma farmacéutica <strong>"${form.value.Nombre}"</strong> con el código <strong>${form.value.Codigo}</strong>?`,
      labelConfirm: 'Sí, crear',
      labelCancel: 'Cancelar',
    })
    if (!confirmed) return

    $loading.show('Creando Forma Farmacéutica...')
    try {
      const payload = {
        Codigo: form.value.Codigo.trim(),
        Nombre: form.value.Nombre.trim(),
      }
      const response = await mercanciaService.formaFarmaceutica.createFormaFarmaceutica(payload)
      if (response.data?.success) {
        mercanciaService.formaFarmaceutica.invalidateFormasFarmaceuticasCache()
        $toast.success('Forma farmacéutica creada exitosamente')
        emit('created')
        emit('update:modelValue', false)
      } else {
        $toast.error(response.data?.message || 'No se pudo crear la forma farmacéutica')
      }
    } catch (error) {
      console.error('Error creando forma farmacéutica:', error)
      if (!error._toastShown) {
        $toast.error('Error inesperado al crear la forma farmacéutica')
      }
    } finally {
      $loading.hide()
    }
