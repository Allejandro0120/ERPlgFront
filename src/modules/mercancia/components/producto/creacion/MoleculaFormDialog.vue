...
<script setup>
  ...
  async function submitForm() {
    ...
    const confirmed = await $confirm.warning({
      ...
      message: `¿Deseas crear la molecula <strong>"${form.value.Nombre}"</strong> con el código <strong>${form.value.Codigo}</strong>?`,
      labelConfirm: 'Sí, crear',
      labelCancel: 'Cancelar',
    })
    if (!confirmed) return

    $loading.show('Creando Molecula...')
    try {
      const payload = {
        Codigo: form.value.Codigo.trim(),
        Nombre: form.value.Nombre.trim(),
      }
      const response = await mercanciaService.molecula.createMolecula(payload)
      if (response.data?.success) {
        mercanciaService.molecula.invalidateMoleculasCache()
        $toast.success('Molecula creada exitosamente')
        emit('created')
        emit('update:modelValue', false)
      } else {
        $toast.error(response.data?.message || 'No se pudo crear la molecula')
      }
    } catch (error) {
      console.error('Error creando molecula:', error)
      if (!error._toastShown) {
        $toast.error('Error inesperado al crear la molecula')
      }
    } finally {
      $loading.hide()
    }
