...
export function useRecepcionDetalles({ isReadonly, permisos }) {
  ...
  function apiDetalleToLocal(api) {
    return {
      ...
      PrecioUnitario: formatCOP(api.PrecioUnitario ?? 0),
    }
  }
  // Solo los campos que van al patch, sin campos de display ni IDs de ubicación
  // PrecioUnitario se normaliza a número para comparaciones y envío a la API
  function detalleSerializable(detalle) {
    return {
      ...pickFields(detalle, PRODUCTO_SNAPSHOT_FIELDS, PRODUCTO_DEFAULTS),
      PrecioUnitario: parseCOP(detalle.PrecioUnitario) ?? 0,
    }
  }
  // Estado local (ya serializado/numérico) → payload para API: solo campos del patch, con ID si se incluye
  ...
  function getDetallesChanges() {
    // Serializado (PrecioUnitario numérico) para comparar y enviar valores correctos a la API
    const serializedCurrent = detalles.value.map((d) => detalleSerializable(d))
    const currentIds = new Set(serializedCurrent.filter((d) => d.IdDetalle).map((d) => d.IdDetalle))
