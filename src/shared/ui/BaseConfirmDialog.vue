<template>
  <v-dialog :model-value="modelValue" :persistent="persistent" max-width="440" transition="dialog-transition"
    ...
    <v-card class="pa-2">
      ...
      <v-card-text class="text-center px-4 pt-0 pb-2">
        <slot name="content">
          <p class="text-body-medium mb-0 text-brand-grey-2">{{ message }}</p>
        </slot>
      </v-card-text>
      ...
      <v-card-actions class="justify-end  mt-2">
        <slot name="actions" :accept="accept" :cancel="cancel">
          <template v-if="actions">
            <v-btn variant="outlined" class="me-2 px-4"  color="outline" height="40" @click="cancel">
              <span style="color: rgb(var(--v-theme-brand-grey-2))">{{ labelCancel }}</span>
            </v-btn>
            <v-btn variant="flat" :color="color" class="px-4" height="40" @click="accept">
              {{ labelConfirm }}
            </v-btn>
          </template>
          <v-btn v-else variant="flat" block height="44" :color="color" @click="accept">
            {{ labelConfirm }}
          </v-btn>
        </slot>
      </v-card-actions>
